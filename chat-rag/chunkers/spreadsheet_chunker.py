...
class SpreadsheetChunker(BaseChunker):
    ...
    def _spreadsheet_process(self):
        ...
        # Support both bytes and file path
        if isinstance(blob_data, str):
            # If blob_data is a file path string, load directly
            logging.debug(f"[spreadsheet_chunker][{self.filename}][spreadsheet_process] Loading from file path: {blob_data}")
            workbook = load_workbook(blob_data, data_only=True)
        else:
            # If blob_data is bytes, use BytesIO stream
            blob_stream = BytesIO(blob_data)
            logging.debug(f"[spreadsheet_chunker][{self.filename}][spreadsheet_process] Starting openpyxl load_workbook from bytes.")                    
